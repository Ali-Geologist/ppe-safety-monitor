...
def show_camera_setup():
    ...
    # TAPO Camera Quick Setup
    st.subheader("🔌 TAPO Camera Setup")
    
    col1, col2 = st.columns([2, 1])
    
    with col1:
        tapo_ip = st.text_input(
            "TAPO Camera IP Address:",
            value="192.168.100.31",
            placeholder="192.168.100.31",
            help="Enter your TAPO camera IP address"
        )
        
        tapo_username = st.text_input(
            "TAPO Username:",
            value="admin",
            placeholder="admin",
            help="Usually 'admin' for TAPO cameras"
        )
        
        tapo_password = st.text_input(
            "TAPO Password:",
            type="password",
            placeholder="Enter your TAPO camera password",
            help="Your TAPO camera password"
        )
    
    with col2:
        st.write("")
        st.write("")
        if st.button("🔗 Generate TAPO URL", use_container_width=True):
            if tapo_ip and tapo_username and tapo_password:
                # Generate RTSP URL for TAPO camera
                tapo_rtsp_url = f"rtsp://{tapo_username}:{tapo_password}@{tapo_ip}:554/stream1"
                st.session_state.generated_tapo_url = tapo_rtsp_url
                st.success("✅ TAPO RTSP URL generated!")
                st.code(tapo_rtsp_url)
                
                # Auto-add to camera list
                if tapo_rtsp_url not in st.session_state.camera_urls:
                    st.session_state.camera_urls.append(tapo_rtsp_url)
                    st.success("✅ TAPO camera added to saved cameras!")
            else:
                st.error("❌ Please fill in all TAPO camera details")
    
    # Manual URL input (existing functionality)
    st.subheader("🌐 Manual Camera URL Setup")
    ...
    with col1:
        camera_url = st.text_input(
            "Camera URL:",
            placeholder="rtsp://username:password@ip:port/stream or http://ip:port/video",
            help="Enter your camera stream URL"
        )
                    ...
                    if CV2_AVAILABLE:
                        try:
                            ...
                            if cap.isOpened():
                                ...
                                cap.release()
                                if ret:
                                    st.success(f"✅ Camera {i+1} working!")
                                else:
                                    st.error(f"❌ Camera {i+1} no frame")
                            else:
                                st.error(f"❌ Camera {i+1} not accessible")
                        except Exception as e:
    ...
    if st.session_state.camera_urls:
        ...
        with col1:
            if st.button("▶️ Start Monitoring", type="primary", use_container_width=True):
                st.session_state.monitoring = True
                st.session_state.selected_camera_url = selected_camera
                st.success("🚀 Starting live monitoring...")
        with col2:
            if st.button("⏹️ Stop Monitoring", use_container_width=True):
                st.session_state.monitoring = False
        ...
        # Show live feed if monitoring is active
        if st.session_state.monitoring and CV2_AVAILABLE:
            show_live_feed(st.session_state.selected_camera_url)
    else:
        st.warning("No cameras configured. Add a camera URL above.")
    
    # TAPO Camera Help Section
    with st.expander("📋 TAPO Camera Help"):
        st.markdown("""
        **TAPO Camera Setup Guide:**
        
        1. **Find your camera IP:** Usually shown in the TAPO app under device info
        2. **Default credentials:**
           - Username: `admin`
           - Password: Your TAPO app password
        3. **Common TAPO RTSP URLs:**
           - `rtsp://admin:password@192.168.100.31:554/stream1`
           - `rtsp://admin:password@192.168.100.31:554/stream2` (higher quality)
        
        **Troubleshooting:**
        - Ensure camera is on the same network
        - Check if RTSP is enabled in TAPO app settings
        - Verify username/password are correct
        - Make sure port 554 is not blocked by firewall
        """)
